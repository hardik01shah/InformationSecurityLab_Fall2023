...
L = num_leaked_bits
n = max_querries
B = matrix.identity(ZZ, max_querries+1) * schnorr.q
B[-1,-1] = 1
...
for i in range(max_querries):
    json_send({"command": "get_signature", "msg": f"gimme the flag {i}"})
    response = json_recv()
    h = schnorr.Z_q(response["h"])
    s = schnorr.Z_q(response["s"])
    alpha = response["nonce"]>>(256 - num_leaked_bits)
    u = alpha*(2**(N - L)) + 2**(N - L - 1) - s
